...
@Composable
fun MainTestScreen(baseViewModel: BaseViewModel, mainViewModel: MainBaseViewModel, viewModel: MainTestScreenViewModel) {
    ...
    ConstraintLayout(
        modifier = Modifier
            .fillMaxSize()
            .background(color = Color(0xFFF4F2EE)) // 배경색 지정
    ) {
        // Constraints 정의
        ...
        // 환영 메시지
        Text(
            text = "Welcome to Main Test Screen!",
        ...
        // Balance 텍스트
        Text(
            text = "Main TEST Balance: $balance",
